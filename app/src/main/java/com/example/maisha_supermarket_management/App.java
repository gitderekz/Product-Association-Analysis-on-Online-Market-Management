...
public class App extends Observable{
    ...
    @RequiresApi(api = Build.VERSION_CODES.N)
    public  App(String[] args, Observer ob) throws Exception
    {
        usedAsLibrary = true;
        configure(args);
        this.addObserver(ob);
...
//        if (args.length!=0) transaFile = args[0];
        if (args.length!=0) transaFile = Environment.getExternalStorageDirectory()+File.separator+"MaishaSupermarket"+File.separator+"Dataset"+File.separator+fileName;
        else transaFile = Environment.getExternalStorageDirectory()+File.separator+"MaishaSupermarket"+File.separator+"Dataset"+File.separator+fileName; // default
...
//                        while(writer.ready()){
//                            String line= String.valueOf(writer.read());
//                            if (line.matches("\\s*")) continue; // be friendly with empty lines
//                            numTransactions++;
//                            StringTokenizer t = new StringTokenizer(line," ");
//                            while (t.hasMoreTokens()) {
...
////                        writer.flush();
////                        writer.close();
//                        Log.d("NOERRORS","Data has been written to Dataset");
...
//                        Log.d("ERRORS2","Failed 02");
//                    }
//                }else{
//                    File gpxfile = new File(root+File.separator+fileName);
//                    FileReader writer = new FileReader(gpxfile);
//                    while(writer.ready()){
//                        String line= String.valueOf(writer.read());
//                        if (line.matches("\\s*")) continue; // be friendly with empty lines
...
//                Log.d("ERRORS","Failed to write to Dataset");
//            }
...
//                    File gpxfile = new File(root, fileName);
//
//                    FileReader data_in = new FileReader(gpxfile);
//        ///ooooopppppppppooooooo////
...
//            String line = data_in.readLine();
            String line = String.valueOf(data_in.readLine());
    ...
    public static void Lister() {
        ...
        if (m == 0) {
            ...
            new SharebleResources("No association rules passed minimum confidence of "+minConf * 100,"");
        }
    }
    public static int assoc_print(int[] a, int[] b, int a1, int b1) {
        ...
        if (k == 0) {
            ...
            if (Lol > minConf) {
                ...
                new SharebleResources(win+"==>"+lose,String.valueOf(Lol*100));
                return 1;
            }
        }
        return 0;
